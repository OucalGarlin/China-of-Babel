	...
	local items = {
		{
			...
			{ ABY = "不会造成伤害的蝗虫, 碰撞时迸射火焰",
			ACR="房间中央的魅惑蜡烛, 灼烧碰到的敌人",
			...
			ACR="杀死变种以充能, 为泪弹赋予变种效果",
			ABY="蝗虫在与泪弹碰撞后有概率为其赋予随机特效"}
		},{
			"Jar of Maggots",
			"蛆罐",
			"你还怕蛆啊你个废物",
			...
			{ABY="碰撞后召唤{{Trinket115}} 减速蝗虫的深渊蝗虫",
			ACR="概率发射水袋, 迸射泪弹"}
		},{
			"Landmines",
			"地雷",
			...
			ABY="5只2.5倍伤害的蝗虫, 碰撞后有概率生成{{Collectible512}} 黑洞"}
		},{
			"Sulphur",
			"炼金硫火",
			"你知道是什么让我成为一块好石头吗?",
			"!!! 随机自爆#不会伤害自己",
			{ABY="飞行时会随机爆炸的蝗虫(放心没有友伤)",
			ACR="随机时刻自爆"}
		},{
			"Mercury",
			"炼金汞",
			...
			"蝇蛆先生的巧克力",
			"有股\"这是内脏\"味!",
			"有1/20的概率射出一个会施加{{Weakness}} 虚弱效果的友方蝇蛆",--大概率捏他了Mr. Beast Feastables巧克力
			{ABY="概率生成 会施加虚弱效果的小蝇蛆 的蝗虫",
			ACR="概率发射虚弱蝇蛆"}
		},{
			"Fusion",
			"融合",
			...
			BEL="每次改变卡牌都会使其爆炸",
			ACR="塔罗牌编号+1",
			ABY="蝗虫碰到{{Card}} 塔罗牌后会将其消耗并立刻触发效果"}
		},{
			...
			{ABY="蝗虫有极小的概率触发{{Pill}}好困或{{Pill}}好兴奋的效果",
			ACR="碰撞后改变时间流速的跟班"}
		},{
			"Potted Protector",
			"盆栽护卫",
			...
			BEL="同时对目标敌人发射激光炮",
			ACR="虚弱编号最高的敌人",
			ABY="蝗虫会秒杀碰到的战5渣(然后失去该蝗虫), 释放传说中的超{{Trinket}}级{{Trinket}}爆{{Trinket}}炸然后开始插入旁白"}
		},{
			"Grandnade",
			"壮丽炸弹",
			"挥霍资产",
			...
			{ABY="命中敌人后释放一圈Y个泪弹的蝗虫#(Y = 游戏时间/10 的余数)",
			ACR="下层获得硬币, 越快越好"}
		},{
			...
			{MOD={GOLD=true},
			ACR="吞噬物品 + 概率悬赏随机物品, 完成有奖励"}
		},{
			"Pepper Steak",
			"胡椒牛排",
			"好像是OFF",
			"↑ +1 伤害#!!! 50% 的概率在拾取后失去5炸弹#{{GarlinIcon}} 可能由于某些漏洞导致永远失去炸弹!",
			{ABY="3只0.2倍伤害的蝗虫",
			ACR="伤害上升 + 炸弹下降"}
		},{
			"Dice Bombs",
			...
			{ABY="命中敌人后定住并延时爆炸的蝗虫, 会吃掉生命值低于5的敌人, 有4%的概率生成一个饰品",
			ACR="被炸后吞下饰品 + 5个炸弹"}
		},{
			"Pepper Steak?",
			"胡椒牛排?",
			...
			{ABY="如果当前房间内使用过主动道具, 这只蝗虫会在当前房间内生成4个自己的复制",
			ACR="消耗炸弹获得伤害上升"}
		},{
			...
			{ABY="悬浮在房间中央的蝗虫, 在角色攻击时造成碰撞伤害",
			ACR="五花八门的广告! + 可选安全卫士 + 小心病毒弹窗!",
			...
			{ABY="2只仅有0.1倍伤害的蝗虫, 命中敌人后必定施加{{Collectible398}}缩水效果",
			ACR="虚弱最近的敌人, \"无视\"其他敌人!"}
		},{
			...
			BEL="同时释放爆炸, 造成1.2倍伤害",
			ABY="这只蝗虫可以为你提供金掉落物, 但你用它造成的伤害得等同于澳大利亚每克黄金的价值!#{{CR}}该数值在2024/11/15为127$",
			ACR="充能型节奏点金冲击波",
